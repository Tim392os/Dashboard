...
| **École — saisie rapide (PWA)** | ✅ Fonctionnel | Cabanga n'a pas d'API publique : la carte École est en saisie locale — notes par matière (avec date et barème libre), devoirs et contrôles (ajout ＋, coche = fait), temps d'étude. Graphique d'évolution : courbe de moyenne générale cumulée + une courbe par matière (puces sélectionnables, couleur fixe par matière). |
